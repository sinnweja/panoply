...
#match.index <- which(clinPanCO$Vital.Status=="Alive" & clinPanCO$PatientID!=patient & clinPanCO$age_at_diagnosis>60 & clinPanCO$age_at_diagnosis<=70 & clinPanCO$T.stage >= 2 & clinPanCO$N.stage != "N2" & clinPanCO$ndays > 1000)
...
set.seed(1000)
drugTable <- panDrugSets(drivGenes, caseids=patient, controlids=ptmatch, gcount=gcPanCO,nsim=1000, gene.gs=reactome.gs, gene.adj=reactome.adj, drug.gs=drug.gs, drug.adj=drug.adj, tailEnd=tailEnd)
...
panCircos(drivGenes, drugTable[drugTable$N.Pathways>6,], caseids=patient,  variant=NULL, cnaPanCO, gcount=gcPanCO, gcinfo=gcinfoPan,
      tailEnd=tailEnd,tailPct=tailPct, minTargets=minDrugTargets, minPathPct=minPathPct, minPathSize=)
...
if(FALSE){
 panGeneGraph <- function (panGene, panDrug, minTargets = 2, minPathways = 2, 
    ndrugs = 4, ndrivers = 20, ...)  {
    requireNamespace("Rgraphviz")
    data(react.graph)
    drTable <- panDrug[with(panDrug, !is.na(N.Pathways) & N.Pathways >= 
        minPathways & !is.na(N.Network.Genes) & N.Network.Genes >= minTargets), ]
    ...
    sgAttrs$fillcolor = rep(rgb(128/256, 177/256, 211/256), length(nodes(sg1)))
    names(sgAttrs$fillcolor) = nodes(sg1)
    sgAttrs$fillcolor[intersect(zdr, nodes(sg1))] = rgb(251/256, 
        128/256, 114/256)
    sgAttrs$shape = rep("box", length(nodes(sg1)))
    names(sgAttrs$shape) = nodes(sg1)
    sgAttrs$shape[unique(c(zconn, zdr.target))] = "circle"
    sgAttrs$shape[intersect(zdr.target, nodes(sg1))] = "ellipse"
...
<<drugview, echo=FALSE, eval=FALSE>>=
